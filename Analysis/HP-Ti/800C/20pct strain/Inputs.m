...
EBSDinterface='osc';
% EBSDinterface='ang';
EBSDfname = fullfile(dataDir,strcat('800C_20pct_ND_Cleaned.',EBSDinterface));
...
MistolRlx=8*degree; %Used for manually added grains in cluster and cluster internal grains
minNEdgeMistol=2;
%% Twin boundary tolerances
meanMistol=5*degree; %needs to be slight larger (like Rod noted) 
meanMistolRelaxed=10*degree; %used when typing edges
...
%   tnum=5;
%  twin{tnum}.CS=CS{2};
%  twin{tnum}.name='test <30-32>(10-13)';
%  twin{tnum}.k1=Miller(-1,0,-1,3,twin{tnum}.CS,'hkl'); %What you specify here affects sign and schmid value
%  twin{tnum}.eta1=Miller(3,0,-3,2,twin{tnum}.CS,'uvtw'); %What you specify here affects sign and schmid value
%  twin{tnum}.actType=1; %(pick 1-4):180 around K1, 180 around eta1, compound - K1 then eta1, compound - eta1 then K1
%  twin{tnum}.variantsToUse=1; 
%  
%    tnum=6;
%  twin{tnum}.CS=CS{2};
%  twin{tnum}.name='test <11-22>(11-23)';
...
%  tnum=7;
%  twin{tnum}.CS=CS{2};
%  twin{tnum}.name='test <11-24>(22-43)';
%  twin{tnum}.k1=Miller(-1,-1,2,4,twin{tnum}.CS,'hkl'); %What you specify here affects sign and schmid value
%  twin{tnum}.eta1=Miller(2,2,-4,3,twin{tnum}.CS,'uvtw'); %What you specify here affects sign and schmid value
%  twin{tnum}.actType=1; %(pick 1-4):180 around K1, 180 around eta1, compound - K1 then eta1, compound - eta1 then K1
%  twin{tnum}.variantsToUse=1; 
