...
groups=unique(find(err_group))
value=G_Complete.Nodes.meanOrientation;
plotNeighbors=true;
enforceClusterOnlyMod=false;
[G_Complete,~,~] = ClusterEditor(groups,G_Complete,grains,mergedGrains,value,0,plotNeighbors,enforceClusterOnlyMod); 
...
plot(mergedGrains.boundary,'lineWidth',2,'lineColor','w');hold off
mtexTitle('Twin Type');mtexColorbar;
print('twin_type','-dtiffn','-r300');
figure; plot(grains_fin,grains_fin.prop.Generation);hold on
plot(mergedGrains.boundary,'lineWidth',2,'lineColor','w');hold off
mtexTitle('Twin Generation');mtexColorbar;
