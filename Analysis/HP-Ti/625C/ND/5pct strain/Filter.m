...
if tFilter.PARIS.use
    ...
    for i=1:max(G.Edges.Group)
        ...
        for j=1:nFamilies2Remove
            ...
            if length(e2Remove)==1 & tFilter.singleFragRelationship
                n1=ePairs(e2Remove,1);
                n2=ePairs(e2Remove,2);
                
                %We should ignore internal grains and edge types that
                %are excluded
                if ~any(n1==GrainIdInclusion | n2==GrainIdInclusion) && any(eType(e2Remove)==tFilter.twinModes)
    ...
    parfor cnt=1:size(toCombine,2)
        toCombine_local=toCombine(:,cnt);
        [mergedGrains2,parentId2,combinedTwinBoundary,combine] = MergeByEdge(G.Edges.pairs,toCombine_local,grains);
        ...
        for i=1:max(G.Edges.Group)
            ...
            if any(~toCombine_local(egroupId))
                ...
                parisChange=(paris_orig(nId_orig)-mean_paris_split)/paris_orig(nId_orig);
                
                
                if parisChange<tFilter.PARIS.value/100 | ~tFilter.PARIS.use
                    %Not enough improvement to keep edge removal
                    toCombine_local(egroupId)=true;
                end
            end
        end
        toCombine(:,cnt)=toCombine_local
    end
    
    %     toremove=ones(length(G.Nodes.Id),1,'logical');
    %     toremove(unique(G.Edges.pairs([any(~toCombine,2),any(~toCombine,2)])))=false;
    %     G_nRemoved=rmnode(G,find(toremove));
    %     %Overlayer clustered grains graph on grains and label edges
    %     figure;
    %     plot(grains(G_nRemoved.Nodes.Id),...
    %         G_nRemoved.Nodes.meanOrientation,'Micronbar','off','silent');
    %Remove edges
    G=removeEdge(G,any(~toCombine,2),1:length(toCombine));
    
    [mergedGrains2,parentId2,combinedTwinBoundary,combine] = MergeByEdge(G.Edges.pairs,G.Edges.combineCleaned,grains);
    figure;
    plot(grains,grains.meanOrientation);hold on;
    plot(mergedGrains2.boundary,'linecolor','w','linewidth',2,'linestyle','-',...
